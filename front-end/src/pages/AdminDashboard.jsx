...
export default function AdminDashboard() {
  ...
  const filteredUsers = users.filter(u => u.name && u.name.toLowerCase().includes(searchTerm.toLowerCase()));
  const filteredOrders = orders.filter(o => o.customer && o.customer.toLowerCase().includes(searchTerm.toLowerCase()));
                    ...
                    <tr key={p.id}>
                      <td>
                        <div style={{display: 'flex', alignItems: 'center', gap: '0.75rem'}}>
                          <img src={p.image || 'https://via.placeholder.com/50'} alt={p.name} className="product-img" />
                      ...
                      <td>
                        <span className={`badge ${p.stock > 10 ? 'success' : 'warning'}`}>{p.stock}</span>
                      </td>
                      <td>
                ...
                <tbody>
                  {filteredUsers.map(u => (
                    <tr key={u.id}>
                      ...
                      <td>{u.email}</td>
                      <td>{u.joined || 'N/A'}</td>
                      <td><span className="badge primary">{u.orders || 0}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
