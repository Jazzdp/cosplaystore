...
export default function AccountSettings() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setProfile({ email: data.email || '', fullName: data.fullName || '' });
      } catch (err) {
        setError(String(err));
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('user', JSON.stringify({ ...user, email: updated.email, fullName: updated.fullName }));
      navigate('/');
    } catch (err) {
      setError(String(err));
    ...
    <div style={{minHeight:'80vh',padding:'40px 20px',background:'linear-gradient(135deg,#fdf2f8 0%,#fce7f3 50%,#fbcfe8 100%)'}}>
      <div style={{maxWidth:900,margin:'0 auto',display:'grid',gridTemplateColumns:'1fr 340px',gap:24}}>
        ...
        <aside style={{background:'white',padding:20,borderRadius:12,boxShadow:'0 10px 30px rgba(0,0,0,0.06)'}}>
          ...
          <p style={{color:'#6b7280'}}>Update your name, email or password here. Changes will be applied to your account immediately.</p>
        </aside>
      </div>
    </div>
  );
